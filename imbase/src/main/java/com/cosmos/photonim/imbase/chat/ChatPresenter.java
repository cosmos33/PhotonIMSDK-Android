...
import com.cosmos.photonim.imbase.chat.custom.CustomActivity;
import com.cosmos.photonim.imbase.chat.custom.CustomInfo;
...
public class ChatPresenter<V extends IChatView, M extends IChatModel> extends IChatPresenter<V, M> {
    ...
    private static HashMap<String, ChatData> chatMsgMap;//key:msgId
...
//    }
    ...
    public void sendChannelMsg(String from,String to,int arg1,int arg2,byte[] data,boolean sync){
        getiModel().sendChannelMsg(from,to,arg1,arg2,data,sync);
    }
    private void sendFile(ChatData chatData) {
        if (chatData.isLocalFileToLarge()) {
            getIView().toast("文件超过100M，暂不支持发送");
            return;
        }
        ...
        if (TextUtils.isEmpty(chatData.getFileUrl())) {// TODO: 2020-02-05 maybe judge file is exist?
            getiModel().uploadFile(chatData, new IChatModel.OnFileUploadListener() {
                ...
                public void onFileUpload(boolean success, ChatData chatData, PhotonIMMessage photonIMMessage) {
                    this.onFileUploadWithSendMsg(success, chatData, photonIMMessage, true);
                }

                @Override
                public void onFileUploadWithSendMsg(boolean success, ChatData chatData, PhotonIMMessage photonIMMessage, boolean sendMsg) {
    ...
    private void notifyItemPorgress(ChatData chatData, int progress) {
        ...
        CustomRunnable customRunnable = new CustomRunnable.Builder()
                .runnable(new Runnable() {
                    ...
                    public void run() {
                        if (chatMsgMap != null) {
                            ChatData cd = chatMsgMap.get(chatData.getMsgId());
                            if (cd != null) {
                                cd.setProgress(progress);
                                cd.setShowProgress(true);
    ...
    public void onReceiveMsg(PhotonIMMessage msg) {
        ...
        if (msg.status == PhotonIMMessage.RECV_READ && msg.msgIds != null) {
            for (String msgId : msg.msgIds) {
...
//        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == Constants.REQUEST_IMAGE_CODE && resultCode == Activity.RESULT_OK) {
            ...
            sendFile(data);
        } else if (requestCode == Constants.REQUEST_CUSTOM && resultCode == Activity.RESULT_OK){
            sendCustom(data);
        }
    }
...
//        getiModel().removeChat(chatType, chatWith, id);
    ...
    @Override
    public M generateIModel() {
        return (M) new ChatModel();
    }
    ...
    private void sendFile(Intent data) {
        ...
        for (FileItemData file : result) {
            if(file.getSizeL() > 100*1024*1024){
                ToastUtils.showText("文件大于100M,IMSDK暂不支持发送");
                return;
            }
    ...
    private void sendCustom(Intent data){
        CustomInfo customInfo = (CustomInfo)data.getSerializableExtra(CustomActivity.INTENT_CUSTOM);
        ChatData.Builder chatDataBuild = new ChatData.Builder()
                .icon(myIcon)
                ...
                .from(loginUserId)
                .to(chatWith)
                .customArg1(customInfo.customArg1)
                .customArg2(customInfo.customArg2)
                .customData(customInfo.customData)
                .customDataSize(customInfo.customDateSize);
        sendMsg(chatDataBuild);
    }
