...
import com.momo.demo.main.service.MusicActivity;
...
//        baseDataMap.clear();
    ...
    public void onLogClick(){
        Intent intent = new Intent(AppContext.getAppContext(), MusicActivity.class);
        intent.putExtra("url","http://music.163.com/song/media/outer/url?id=1448025691.mp3");
        AppContext.getAppContext().startActivity(intent);
//        ImBaseBridge.getInstance().upLoadMMfileLog();
    }

    @OnClick(R2.id.tvAuth)
    public void onAuthClick(){
        if(presenter.getCurrentIMLoginStatus()){
            ToastUtils.showText("CurrentIMLoginStatus：Success");
        }else{
            ToastUtils.showText("CurrentIMLoginStatus：Failed");
        }
    }
