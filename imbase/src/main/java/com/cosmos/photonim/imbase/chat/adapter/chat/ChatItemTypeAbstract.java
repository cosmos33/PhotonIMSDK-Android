...
import com.cosmos.codedlib.Base64;
    ...
    protected void fillMsgContent(RvViewHolder rvViewHolder) {
        ...
        switch (chatData.getMsgType()) {
            case PhotonIMMessage.RAW:
                if(chatData.getChatType() == PhotonIMMessage.ROOM){
                    rvViewHolder.getView(R.id.llMsgRoot).setVisibility(View.GONE);
                    view.setVisibility(View.VISIBLE);
                    try {
                        ((TextView) view).setText(new String(chatData.getData()));
                    }catch (Exception e){}
                }else{
                    setVisible(rvViewHolder, R.id.tvContent);
                    TextView content1 = (TextView) rvViewHolder.getView(R.id.tvContent);
                    content1.setVisibility(View.VISIBLE);
                    try{
                        content1.setText("arg1:"+chatData.getCustomArg1() + " "+"arg2:"+chatData.getCustomArg2()+" data:"+ new String(chatData.getCustomData()) +" size:"+chatData.getCustomDataSize());
                    }catch (Exception e){
                        e.printStackTrace();
                        content1.setText("发送格式错误，可恶");
                    }
                }
