...
public class IMReceiveHelper {
    ...
    public void start() {
        ...
        PhotonIMClient.getInstance().setPhotonIMCommonListener(new PhotonIMClient.PhotonIMCommonListener(){
            @Override
...
//            }
        ...
        PhotonIMClient.getInstance().setPhotonIMExceptionListener((int ec,String msg) -> {
            LogUtils.log("PIM","exception:"+ec+" msg:"+msg);
        });
...
//        });
        ...
        PhotonIMClient.getInstance().setPhotonIMMessageReceiver(new PhotonIMClient.PhotonIMMessageReceiver(){
            @Override
            public void onReceiveMessage(PhotonIMMessage message, String lt, long lv){
                LogUtils.log("PIM", "recv a ( "+message.chatType+" ) msg :onReceiveMessage");
                EventBus.getDefault().post(message);
            }
            @Override
            public void onReceiveReadMessage(PhotonIMMessage message){
                LogUtils.log("PIM", "recv a msg :onReceiveReadMessage");
                EventBus.getDefault().post(message);
            }

            @Override
            public void onReceiveRecallMessage(PhotonIMMessage message){
                LogUtils.log("PIM", "recv a msg :onReceiveRecallMessage");
                EventBus.getDefault().post(message);
            }

            @Override
            public void onReceiveDelMessage(PhotonIMMessage message){
                LogUtils.log("PIM", "recv a msg :onReceiveDelMessage");
            }

            @Override
            public void onReceiveRoomMessage(PhotonIMMessage message){
                LogUtils.log("PIM", "recv a msg :onReceiveRoomMessage");
                EventBus.getDefault().post(message);
            }
        ...
        extra.put("biz1","biz_extra1");
        extra.put("biz2","biz_extra2");

        PhotonIMClient.getInstance().login(ImBaseBridge.getInstance().getUserId(), ImBaseBridge.getInstance().getTokenId(), extra);
