...
//        ToastUtils.showText(this, status.statusMsg);
    ...
    public void onReceiveMsg(PhotonIMMessage msg) {
        if (msg.chatType == PhotonIMMessage.CUSTOMMSG) {
            ...
            if(body.arg1 == 3){
                NotifyHelper.INSTANCE.sendNotify(AppContext.getAppContext(),"关注",msg.from+"关注了你",true,true);
            }else if(body.arg1 == 4){
                NotifyHelper.INSTANCE.sendNotify(AppContext.getAppContext(),"关注",msg.from+"取消关注了你【哭】",true,true);
            }else if(body.arg1 == 1 || body.arg1 == 2){

            }else{
                ToastUtils.showText(String.format("系统自定义消息：customArg1:%d,customArg2:%d", body.arg1, body.arg2));
            }
        }
    }
