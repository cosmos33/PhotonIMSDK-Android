...
import com.cosmos.photonim.imbase.chat.filehandler.SDKFileHandler;
...
public class ChatModel extends IChatModel {
    ...
    public ChatModel() {
        iFileHandler = FileHandlerProxy.getiFileHandler();
    }
...
//        List<PhotonIMMessage> list = PhotonIMDatabase.getInstance().findMessageListByMsgType(chatType,chatWith,null,0,0, TextUtils.isEmpty(anchorMsgId) ?null:anchorMsgId,size,beforeAuthor);
        PhotonIMDatabase.Parameters parameters = new PhotonIMDatabase.Parameters();
        parameters.anchorMsgId = TextUtils.isEmpty(anchorMsgId)?null:anchorMsgId;
        parameters.beforeAnchor = beforeAuthor;
        parameters.size = size;
        List<PhotonIMMessage> list = PhotonIMDatabase.getInstance().findMessageList(chatType,chatWith,parameters);
        ArrayList<PhotonIMMessage> photonIMMessages = (ArrayList<PhotonIMMessage>) list;
        return convertMap(photonIMMessages);
    }

    private Object convertMap(List<PhotonIMMessage> photonIMMessages) {
        ...
        for (PhotonIMMessage photonIMMessage : photonIMMessages) {
            ...
            if((photonIMMessage.messageType == PhotonIMMessage.IMAGE || photonIMMessage.messageType == PhotonIMMessage.AUDIO ||
                    photonIMMessage.messageType==PhotonIMMessage.VIDEO || photonIMMessage.messageType == PhotonIMMessage.FILE) && firstStart){
                if (photonIMMessage.status == PhotonIMMessage.SENDING ) {//从数据库读取的消息状态如果是发送中，转换为发送失败
                    msgStatus = PhotonIMMessage.SEND_FAILED;

                    PhotonIMDatabase.getInstance().updateMessageStatus(photonIMMessage.chatType,photonIMMessage.chatWith,photonIMMessage.id,PhotonIMMessage.SEND_FAILED );
                    firstStart = false;
                }
            }
            String fileHash = "";
...
//                    fileHash = photonIMImageBody.hash;
//                    hashAlg = photonIMImageBody.hashAlg;
                    break;
                case PhotonIMMessage.AUDIO:
                    PhotonIMAudioBody photonIMAudioBody = (PhotonIMAudioBody)photonIMMessage.body;
//                    fileHash = photonIMAudioBody.hash;
//                    hashAlg = photonIMAudioBody.hashAlg;
                    break;
                case PhotonIMMessage.FILE:
                    PhotonIMFileBody photonIMFileBody = (PhotonIMFileBody)photonIMMessage.body;
                    fileHash = photonIMFileBody.hash;
                    hashAlg = photonIMFileBody.hashAlg;
                    break;
                case PhotonIMMessage.VIDEO:
                    PhotonIMVideoBody photonIMVideoBody = (PhotonIMVideoBody)photonIMMessage.body;
//                    hashAlg = photonIMVideoBody.hashAlg;
//                    fileHash = photonIMVideoBody.hash;
                    break;
            }
...
//    }
    ...
    private Object getAfterSearchMsgId(int chatType, String chatWith, String anchorMsgId, boolean beforeAuthor, boolean asc, int size) {
        ...
        List<PhotonIMMessage> result = PhotonIMDatabase.getInstance().findMessageListByIdRange(chatType, chatWith, anchorMsgId, beforeAuthor, asc, size);
...
//                });
        ...
        PhotonIMDatabase.getInstance().syncHistoryMessagesFromServer(chatType, chatWith, size, beginTimeStamp, endTimeStamp, new PhotonIMDatabase.SyncHistoryListener() {
            @Override
            public void onSyncHistory(int i, String s, String s1, List<PhotonIMMessage> list) {
                if(list == null){
                    listener.onLoadHistory(null,null);
                }else{
                    Map map = (Map)convertMap((ArrayList)list);
                    listener.onLoadHistory((List<ChatData>) (map.get("list")), (Map<String, ChatData>) (map.get("map")));
                }
            }
        });
    ...
    private Object getAllHistoryFromServer(int chatType, String chatWith, String anchor, int size, long beginTimeStamp, long endTimeStamp) {
        ArrayList<PhotonIMMessage> resultList = new ArrayList<>();
        PhotonIMDatabase.SyncHistoryResult result = PhotonIMDatabase.getInstance().syncHistoryMessagesFromServer(chatType, chatWith, TextUtils.isEmpty(anchor)?null:anchor, size, beginTimeStamp, endTimeStamp);
        if (result == null) {
            return null;
        }
        if (result.ec == 0) {
...
//                PhotonIMDatabase.getInstance().saveMessageBatch(chatType, chatWith, result.syncMsgList);
        ...
        LogUtils.log("PIM","synchistoryMsgFromServer --> msg.size = "+result.syncMsgList.size());
        return convertMap(resultList);
    }
...
//
    ...
    private Object sendMsgInner(boolean update, ChatData chatData, OnMsgSendListener onMsgSendListener) {
        ...
        if(!chatSetModel.getSaveStatus(PhotonIMMessage.SINGLE,message.from,message.chatWith)){
            message.unSaveMessage();
        }
        if(!chatSetModel.getPushStatus(PhotonIMMessage.SINGLE,message.from,message.chatWith)){
            message.unSendPush();
        }
        if(!chatSetModel.getHistoryStatus(PhotonIMMessage.SINGLE,message.from,message.chatWith)){
            message.unSaveMessageOnServer();
        }
        if(!chatSetModel.getIncreaseUnreadStatus(PhotonIMMessage.SINGLE,message.from,message.chatWith)){
            message.unIncreaseUnRead();
        }

        if(!chatSetModel.getFocusUserStatus(chatData.getChatType(),message.from,message.chatWith)){
            message.unUpdateSessionOrderId();
        }

        PhotonIMDatabase.getInstance().saveOrUpdateMessage(message);
        if(chatSetModel.getSendTimeoutStatus(message.chatType,message.from,message.chatWith) || message.chatType == PhotonIMMessage.ROOM){
            PhotonIMClient.getInstance().sendMessage(message,5*1000,(code,msg,retTime)->{
                EventBus.getDefault().post(new ChatDataWrapper(chatData,code,msg));
            });
        }else{
            PhotonIMClient.getInstance().sendMessage(message,(code,msg,retTime) -> {
                EventBus.getDefault().post(new ChatDataWrapper(chatData,code,msg));
            });
        }
    ...
    @Override
    public void sendChannelMsg(String from,String to,int arg1,int arg2,byte[] data,boolean sync){
        TaskExecutor.getInstance().createAsycTask(()->{
            PhotonIMCustomBody photonIMCustomBody = new PhotonIMCustomBody();
            photonIMCustomBody.arg1 = arg1;
            photonIMCustomBody.arg2 = arg2;
            photonIMCustomBody.data = data;
            photonIMCustomBody.size = data.length;
           PhotonIMClient.getInstance().sendChannelMessage(from,to,sync,false,5*1000,photonIMCustomBody,(ec,em,retTime)->{
